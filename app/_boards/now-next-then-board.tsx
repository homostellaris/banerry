...
const STYLE_OPTIONS: Array<{
...
}> = [
	{
		value: 'studio-ghibli',
		label: 'Studio Ghibli',
		description: 'Digital art with vibrant colors and soft lighting',
	},
	{
		value: 'play-doh',
		label: 'Play-Doh',
		description: 'Soft tactile textures with vibrant colors',
	},
	{
		value: 'ladybird',
		label: 'Ladybird Books',
		description: 'Classic 1960s British illustration style',
	},
	{
		value: 'symbols',
		label: 'Symbols (AAC)',
...
export function NowNextThenBoard({
	...
	useEffect(() => {
		...
		if (
			savedStyle &&
			['studio-ghibli', 'play-doh', 'ladybird', 'symbols'].includes(savedStyle)
		) {
	...
	const handleGenerateAllImages = async () => {
		...
		try {
			const effectiveAvatarImageUrl =
				selectedStyle === 'symbols' ? undefined : (avatarImageUrl ?? undefined)
			const result: GenerateBoardImagesResult = await fetch(
				'/api/generate-board-images',
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
			...
			if (!result.success) {
				...
				if (result.partialImages && result.partialImages.length > 0) {
					...
					toast.info(
						`Saved ${result.partialImages.length} images before the error`,
					)
					onBoardUpdate?.()
				}
				return
		...
		<div className="space-y-6">
			...
			{!readOnly && hasAvatar && (
				<div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
					...
					<span>
						Using learner&apos;s avatar for consistent character appearance
					</span>
				</div>
			)}
