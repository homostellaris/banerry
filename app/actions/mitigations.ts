...
export async function generateMitigations(scriptText: string): Promise<Mitigation[]> {
  ...
  try {
    ...
    if (!apiKey) {
      throw new Error("OpenAI API key is not configured. Please add your OPENAI_API_KEY to the environment variables.")
    }
    ...
    throw new Error(`Failed to generate mitigations: ${error instanceof Error ? error.message : String(error)}`)
  }
}
