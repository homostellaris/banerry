describe("Now Next Then Board", () => {
  ...
  it.skip("should render the board structure with three columns", () => {
    ...
    cy.get("body").then(($body) => {
      ...
      if (
        text.includes("Now, Next, Then Board") ||
        text.includes("Now") ||
        text.includes("Next") ||
        text.includes("Then")
      ) {
  ...
  it("should have learner selection dropdown", () => {
    ...
    cy.get("body").then(($body) => {
      ...
      if (
        text.includes("Select Learner") ||
        text.includes("Choose a learner")
      ) {
  ...
  it("should have voice input button", () => {
    ...
    cy.get("body").then(($body) => {
      const hasActiveColumn =
        $body.find('input[placeholder*="Describe"]').length > 0;
